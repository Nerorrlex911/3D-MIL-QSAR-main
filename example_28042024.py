...
for epoch in range(n_epoch):
    ...
    with torch.no_grad():
        ...
        ytest_pred = net.predict(x_test_scaled)
        test_loss.append(mean_absolute_error(y_test,ytest_pred))
        ytrain_pred = net.predict(x_train_scaled)
        train_loss.append(mean_absolute_error(y_train,ytrain_pred))
...
cvresult = cross_val(bags,labels,net)

import csv
with open(f'lrcurve_{lr}_{weight_decay}.csv', mode="w", newline="", encoding="utf-8") as csv_file:
    csv_writer = csv.writer(csv_file)
    csv_writer.writerow(train_loss)
    csv_writer.writerow(test_loss)
    csv_writer.writerow(val_loss)
with open(f'crossval_{lr}_{weight_decay}.csv', mode="w", newline="", encoding="utf-8") as csv_file:
    csv_writer = csv.writer(csv_file)
    csv_writer.writerows(cvresult)
with open(f'model_{lr}_{weight_decay}.pkl', 'wb') as model_file:
...
# 指定要保存的CSV文件名
test_path = f'test_{lr}_{weight_decay}.csv'
# 将数组写入CSV文件
with open(test_path, mode="w", newline="", encoding="utf-8") as csv_file:
    csv_writer = csv.writer(csv_file)
    y_test_csv = np.hstack((y_test.reshape(-1,1),y_test_pred.reshape(-1,1)))
    csv_writer.writerows(y_test_csv)

# 指定要保存的CSV文件名
train_path = f'train_{lr}_{weight_decay}.csv'

# 将数组写入CSV文件
with open(train_path, mode="w", newline="", encoding="utf-8") as csv_file:
    csv_writer = csv.writer(csv_file)
    y_train_csv = np.hstack((y_train.reshape(-1,1),y_train_pred.reshape(-1,1)))
    csv_writer.writerows(y_train_csv)

# 指定要保存的CSV文件名
val_path = f'val_{lr}_{weight_decay}.csv'

# 将数组写入CSV文件
with open(val_path, mode="w", newline="", encoding="utf-8") as csv_file:
    csv_writer = csv.writer(csv_file)
    y_val_csv = np.hstack((y_val.reshape(-1,1),y_val_pred.reshape(-1,1)))
    csv_writer.writerows(y_val_csv)
