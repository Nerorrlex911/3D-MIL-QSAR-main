...
class BaseRegressor:
    ...
    def loss(self, y_pred: Tensor, y_true: Tensor) -> Tensor:
        ...
        Loss
        """ 
        total_loss = nn.L1Loss(reduction='mean')(y_pred, y_true.reshape(-1, 1))
        return total_loss
...
class BaseNet(nn.Module):
    ...
    def fit(self, x: Union[Sequence[Union[Sequence, np.array]], np.array], y: Union[Sequence, np.array],
        ...
        mTrain = as_tensor(m, dtype=torch.float32)
        if yTrain.ndim == 1:
            print(1)
            yTrain = yTrain.reshape(-1, 1)
        optimizer = optim.SGD(self.parameters(), lr=lr, momentum=0.9, nesterov=True, weight_decay=weight_decay)
        mb = get_mini_batches(xTrain, yTrain, mTrain, batch_size=batch_size)
        self.train()
