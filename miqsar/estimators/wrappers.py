...
class MLP(nn.Module):
    ...
    def fit(self, x: Union[Sequence[Union[Sequence, np.array]], np.array],
        ...
        """
        xTrain = as_tensor(x, dtype=torch.float32)
        yTrain = as_tensor(y, dtype=torch.float32)
        if yTrain.ndim == 1:
            print(1)
            yTrain = yTrain.reshape(-1, 1)
        optimizer = optim.SGD(self.parameters(), lr=lr, momentum=0.9, nesterov=True, weight_decay=weight_decay)
        print(yTrain)
        print(xTrain)
        mb = get_mini_batches(xTrain, yTrain, batch_size=batch_size)
        self.train()
        for x_mb, y_mb in mb:
            ...
            self.eval()
            with torch.no_grad():
                valLoss = self.loss_batch(x_val, y_val, optimizer=None)
                val_loss.append(valLoss)
                trainLoss = self.loss_batch(x_train, y_train, optimizer=None)
                train_loss.append(trainLoss)
                avg_val_loss = np.average(val_loss)
                print("验证集的loss:{}".format(avg_val_loss))
                
                earlystopping(val_loss=avg_val_loss,model=self)
